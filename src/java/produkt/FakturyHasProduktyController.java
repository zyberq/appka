...
import produkt.FakturyHasProdukty;
import produkt.util.JsfUtil;
import produkt.util.PaginationHelper;
import produkt.FakturyHasProduktyFacade;
...
public class FakturyHasProduktyController implements Serializable {
    ...
    @EJB
    private produkt.FakturyHasProduktyFacade ejbFacade;
    ...
    public FakturyHasProdukty getSelected() {
        if (current == null) {
            ...
            current.setFakturyHasProduktyPK(new produkt.FakturyHasProduktyPK());
            selectedItemIndex = -1;
        }
        return current;
    }
    ...
    public static class FakturyHasProduktyControllerConverter implements Converter {
        ...
        produkt.FakturyHasProduktyPK getKey(String value) {
            produkt.FakturyHasProduktyPK key;
            String values[] = value.split(SEPARATOR_ESCAPED);
            key = new produkt.FakturyHasProduktyPK();
            key.setProduktyProduktyId(Integer.parseInt(values[0]));
            key.setFakturyFakturyId(Integer.parseInt(values[1]));
            return key;
        }

        String getStringKey(produkt.FakturyHasProduktyPK value) {
