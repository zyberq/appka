...
import produkt.ProduktyHasKategorie;
import produkt.util.JsfUtil;
import produkt.util.PaginationHelper;
import produkt.ProduktyHasKategorieFacade;
...
public class ProduktyHasKategorieController implements Serializable {
    ...
    @EJB
    private produkt.ProduktyHasKategorieFacade ejbFacade;
    ...
    public ProduktyHasKategorie getSelected() {
        if (current == null) {
            ...
            current.setProduktyHasKategoriePK(new produkt.ProduktyHasKategoriePK());
            selectedItemIndex = -1;
        }
        return current;
    }
    ...
    public static class ProduktyHasKategorieControllerConverter implements Converter {
        ...
        produkt.ProduktyHasKategoriePK getKey(String value) {
            produkt.ProduktyHasKategoriePK key;
            String values[] = value.split(SEPARATOR_ESCAPED);
            key = new produkt.ProduktyHasKategoriePK();
            key.setKategorieKategorieId(Integer.parseInt(values[0]));
            key.setProduktyProduktyId(Integer.parseInt(values[1]));
            return key;
        }

        String getStringKey(produkt.ProduktyHasKategoriePK value) {
